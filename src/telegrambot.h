...
signals:
    ...
    void debugLog(QString msgToLog);

public:
    enum TelegramPollMessageTypes : long long
    {
        None                = 0,
        Message             = 1 << 0,
        EditedMessage       = 1 << 1,
    ...
    // User Functions
    TelegramBotUserProfilePhotos getUserProfilePhotos(qint64 userId, int offset = 0, int limit = 0);

    // Chat Functions
    void banChatMember(QVariant chatId, qint64 userId, bool* response = 0);
    void unbanChatMember(QVariant chatId, qint64 userId, bool* response = 0);
    void restrictChatMember(QVariant chatId, qint64 userId, TelegramBotChatPermissions permissions, qint64 untilDate = 0, bool* response = 0);
    void setChatPermissions(QVariant chatId, TelegramBotChatPermissions permissions, bool* response = 0);
    void leaveChat(QVariant chatId, bool* response = 0);
    TelegramBotChat getChat(QVariant chatId);
    QString exportChatInviteLink(QVariant chatId);
    QList<TelegramBotChatMember> getChatAdministrators(QVariant chatId);
    int getChatMembersCount(QVariant chatId);
    TelegramBotChatMember getChatMember(QVariant chatId, qint64 userId);
    ...
    void pinChatMessage(TelegramBotMessage msg, TelegramFlags flags = TelegramFlags::NoFlag, bool* response = 0);
    void pinChatMessage(QVariant chatId, QVariant messageId, TelegramFlags flags = TelegramFlags::NoFlag, bool* response = 0);
    void unpinChatMessage(QVariant chatId, QVariant messageId = 0, bool* response = 0);
    void unpinAllChatMessages(QVariant chatId, bool* response = 0);
    ...
    void forwardMessage(QVariant targetChatId, QVariant fromChatId, qint64 fromMessageId, TelegramFlags flags = TelegramFlags::NoFlag, TelegramBotMessage* response = 0);
    void copyMessage(QVariant targetChatId, QVariant fromChatId, qint64 fromMessageId, QString caption, TelegramFlags flags = TelegramFlags::NoFlag, qint64 *response = 0);
    void deleteMessage(QVariant chatId, qint64 messageId, bool* response = 0);
    void deleteMessage(TelegramBotMessage message, bool* response = 0); //overload for easier access
    ...
    void sendDocument(QVariant chatId, QVariant document, QString filename = "document", QString caption = QString(), int replyToMessageId = 0, TelegramFlags flags = TelegramFlags::NoFlag, TelegramKeyboardRequest keyboard = TelegramKeyboardRequest(), TelegramBotMessage* response = 0);
    ...
    // Payment Functions
//    TelegramBotMessage sendInvoice(QVariant chatId, QString title, QString desription, QString payload, QString providerToken, QString currency, QList<TelegramBotLabeledPrice> prices, qint64 maxTipAmount = 0, QList<qint64> suggestedTipAmounts = QList<qint64>(), QString startParameter = QString(), QString providerData = QString(), QString photoUrl = QString(), qint64 photoSize = -1, qint64 photoWidth = -1, qint64 photoHeight = -1, bool needName = false, bool needPhoneNumber = false, bool needEmail = false, bool needShippingAddress = false, bool sendPhoneNumberToProvider = false, bool sendEmailToProvider = false, bool isFlexible = false, bool disableNotification = false, qint64 replyToMessageId = 0, bool allowSendingWithoutReply = false, TelegramKeyboardRequest keyboard = TelegramKeyboardRequest());
...
private:
    ...
    QHttpMultiPart* handleFile(QString fieldName, QString filename, QVariant file, QUrlQuery& params, QHttpMultiPart* multiPart = 0);
...
{return static_cast<TelegramBot::TelegramPollMessageTypes>(static_cast<long long int>(a) | static_cast<long long int>(b));}
...
{return (static_cast<long long int>(a) & static_cast<long long int>(b)) == static_cast<long long int>(b);}
